locals {
  application_security_groups_list = flatten([for virtual_network_key, virtual_network in var.virtual_network_variables : [for subnet in virtual_network.subnets : [{
    ...
    ] if lookup(virtual_network, "subnet", null) != null
  ])
}

data "azurerm_network_security_group" "network_security_group" {
  for_each            = { for index, network_security_group in local.application_security_groups_list : "${network_security_group.virtual_network_key}-${network_security_group.subnet_name}-${network_security_group.network_security_group_name}" => network_security_group }
...
resource "azurerm_virtual_network" "virtual_network" {
  ...
  dynamic "subnet" {
    ...
    content {
      ...
      security_group   = data.azurerm_network_security_group.network_security_group.network_security_group["${each.key}-${subnet.value.name}-${subnet.value.network_security_group_name}"].id
    }
  }
